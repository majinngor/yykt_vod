...
const service = axios.create({
    baseURL: 'http://117.72.17.55', // api 的 base_url
...
service.interceptors.response.use(response => {
        if (response.data.code == 208) {
            //debugger
            // 替换# 后台获取不到#后面的参数
            let url = window.location.href.replace('#', 'yykt')
            window.location = 'http://117.72.17.55/api/user/wechat/authorize?returnUrl=' + url
            // window.location = 'http://localhost:9000/api/user/wechat/authorize?returnUrl=' + url
        } else {
